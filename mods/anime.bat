...
start /realtime "JuniorCodeos: anime" "%~dp0..\winws.exe" ^
--wf-tcp=80,443,50000-65535 --wf-udp=443,50000-65535 ^
--filter-udp=443 --hostlist="%~dp0..\domeins\general.txt" --dpi-desync=fake ^
--dpi-desync-udplen-increment=10 --dpi-desync-repeats=5 --dpi-desync-udplen-pattern=0xDEADBEEF ^
...
--dpi-desync-fake-quic="%~dp0..\bin\quic_initial_www_google_com.bin" --new ^
--filter-tcp=80 --dpi-desync=fake,split2 --dpi-desync-autottl=5 --dpi-desync-fooling=md5sig --new ^
--filter-tcp=443 --hostlist="%~dp0..\domeins\general.txt" --dpi-desync=fake,split2 ^
--dpi-desync-autottl=1 --dpi-desync-fooling=md5sig --dpi-desync-repeats=5 ^
--dpi-desync-fake-tls="%~dp0..\bin\tls_clienthello_www_google_com.bin" --new ^
--filter-tcp=1935 --filter-udp=1935 --dpi-desync=fake,split2 ^
--dpi-desync-autottl=5 --dpi-desync-fooling=md5sig
